...
void CBorder::CreateBorder(int nBorderSize)
{
    for (int x = 0; x < nBorderSize; x++) {
        m_listBlock.push_back(CBlock(x, 0));
        m_listBlock.push_back(CBlock(x, nBorderSize - 1));
    }
    for (int y = 1; y < nBorderSize - 1; y++) {
        m_listBlock.push_back(CBlock(0, y));
        m_listBlock.push_back(CBlock(nBorderSize - 1, y));
    }
}

void CBorder::Draw()
{
    for (CBlock iBlock : m_listBlock)
    {
        iBlock.Draw();
    }
